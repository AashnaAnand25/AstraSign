...
export default function VoiceToSign({ onBack, onSettings, embedded, onStatusChange, onAddToHistory }: Props) {
  ...
  return (
    <div className="min-h-screen flex flex-col relative overflow-hidden">
      ...
      {/* 3D Realistic Hand View */}
      <div className={`relative z-10 mx-5 mb-6 shrink-0 ${settings.focusMode ? "a11y-focus-content" : ""}`}>
        <div
          className="rounded-3xl overflow-hidden"
          style={{
            background: "linear-gradient(180deg, rgba(10,10,15,0.95) 0%, rgba(15,10,25,0.9) 100%)",
            border: "1px solid hsl(183 100% 50% / 0.3)",
            boxShadow: "0 10px 30px -10px hsl(183 100% 50% / 0.2), inset 0 0 20px hsl(183 100% 50% / 0.1)",
          }}
        >
          <div
          ...
          {phrase?.text ? (
            <div style={{ height: "300px" }} className="flex flex-col relative">
              <div className="absolute inset-0">
                <AvatarScene>
                  <RiggedHands
                    currentWord={signMode === "words" && animationQueue.length > 0 ? animationQueue[queueIndex] : undefined}
              ...
              <p className="absolute bottom-2 left-0 right-0 text-[10px] text-center z-10 font-bold" style={{ color: "hsl(183 100% 50%)", textShadow: "0 2px 4px rgba(0,0,0,0.8)" }}>
                &quot;{phrase.text}&quot;
              </p>
            </div>
          ) : (
            <div className="flex flex-col items-center justify-center min-h-[300px] text-sm px-4 text-center" style={{ color: "hsl(183 100% 50% / 0.8)" }}>
              <p>Enter text or speak — realistic 3D hands will sign here.</p>
              <div style={{ width: "100%", height: "200px" }} className="relative mt-2 opacity-80 pointer-events-none">
                <AvatarScene>
                  <RiggedHands isAnimating={false} position={[0, 1.0, 0]} scale={1.5} />
                </AvatarScene>
              </div>
            </div>
          )}
