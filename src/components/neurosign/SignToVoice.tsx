...
export default function SignToVoice({ onBack, onSettings, focusMode: focusModeProp, embedded, onStatusChange, onAddToHistory }: Props) {
  ...
  // ── Start webcam ────────────────────────────────────────────────────────────
  useEffect(() => {
    onStatusChange?.(isRecording ? "processing" : "ready");
  ...
  return (
    <div className="min-h-screen flex flex-col relative overflow-hidden bg-[#080810]">
      ...
      {/* Top bar */}
      {!embedded && (
        <div className={`relative z-20 flex items-center justify-between px-5 pt-12 pb-3 transition-opacity duration-300 ${focusMode ? "a11y-focus-dim" : ""}`}
      ...
      {/* Mode Toggle */}
      {!embedded && (
        <div className={`relative z-20 flex justify-center mb-4 ${focusMode ? "a11y-focus-dim" : ""}`}>
      ...
      {/* Controls */}
      <div className={`relative z-20 mx-3 mb-6 glass-strong rounded-3xl p-4 shadow-2xl ${focusMode ? "a11y-focus-dim" : ""}`}>
        {isPlaying && (
